# We import llama.cpp - an optimized LLM inference runtime, as well as flask (a tiny app 
...
# We'll replace the {0} value with the user's input.
template = """USER: {0}
ASSISTANT: 
"""
...
# writing, llama.cpp works best with GGUFv3 models quantized down to about 4-5 bits, but will
# work (slowly) with finer models up to the q8 level.
    chat_session = ""
    chat_session += system_prompt

    #llm = Llama(model_path="./airoboros-l2-7b-gpt4-m2.0-gguf.bin", n_gpu_layers=-1, n_threads=4, n_ctx=2048)
    llm = Llama(model_path="./airoboros-l2-7b-2.1.Q4_K_M.gguf", n_gpu_layers=-1, n_threads=4, n_ctx=2048)
    while True:
        ...
        chat_session += template.format(message)
        stream = llm(
            chat_session,
            max_tokens=2048,
            stop=["USER:"],
        ...
        # finished generating.
        ws.send('<END>')
        # We append the text that we received from the model to the conversation session. 
